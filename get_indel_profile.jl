...
function main(ref_fs, out_fs)
    refs = readRef(ref_fs)
    all_lib = collect(keys(refs))
    for i in all_lib
        if isfile("/home/panxiaoguang/Project/Gene_theropy/spCas9_filter/$i.filter.out")
            open("/home/panxiaoguang/Project/Gene_theropy/$out_fs/$i.indelProfile.tsv", "w+") do IO
	            println("call indel for ", i)
                calling(guina(ReadIn("/home/panxiaoguang/Project/Gene_theropy/spCas9_filter/$i.filter.out")), refs[i][114:end], IO)
            end
        end
    end
end
